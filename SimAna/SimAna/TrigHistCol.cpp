...
ClassImp(TrigHistCol);

TrigHistCol::TrigHistCol(SimAna *sim_ana,double thres): sa{sim_ana},threshold(thres){
  InitHists(sa->Max_Energy);
}


void TrigHistCol::InitHists(double max_E){
...
void TrigHistCol::Histadd(TH1F *h){
  // Add a 1D hist.
  hists.push_back(static_cast<TH1 *>(h));
}

void TrigHistCol::Histadd(TH2F *h){
  // Add a 2D hist.
  hists.push_back(static_cast<TH1 *>(h));
}

void TrigHistCol::Print(void){
